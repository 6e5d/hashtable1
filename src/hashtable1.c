...
static uint64_t hashtable1(getidx)(Hashtable1()* table, uint64_t v) {
	uint64_t idx;
	if (table->bitshift == 0) {
		idx = 0;
	} else {
...
static bool get_entry(Hashtable1()* table,
	void* key, uint64_t idx, void** result
) {
	while (1) {
...
bool hashtable1(get)(Hashtable1()* table, void* key, void** result) {
	...
	uint64_t idx = hashtable1(getidx)(table, v);
...
uint8_t hashtable1(contains)(Hashtable1()* table, void* key) {
	void *p;
	bool ret = hashtable1(get)(table, key, &p);
	return ret;
}
...
static uint8_t insert2(Hashtable1()* table, void* key, void* value) {
	uint8_t* p;
	uint64_t v = fnv1a(key, table->size_key);
	uint64_t idx = hashtable1(getidx)(table, v);
